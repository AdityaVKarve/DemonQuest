...
public class Summoner extends Mob{
	private AnimatedSprite up=new AnimatedSprite(SpriteSheet.summoner_up,32,32,3);
	private AnimatedSprite down=new AnimatedSprite(SpriteSheet.summoner_up,32,32,3);
	private AnimatedSprite right=new AnimatedSprite(SpriteSheet.summoner_right,32,32,3);
	...
	public Summoner(int x, int y)
	{
		...
		name = new NamePlate("Evil summoner",false,new Vector2i(x,y));
		name.isActive = true;
		eui.addItem(name);
	...
	private void spawnMinion() {
		...
		if(System.currentTimeMillis()-clock >= 2500) {
			if(Math.sqrt(Math.pow(PlayerPosition.x -x, 2) + Math.pow(PlayerPosition.y -y,2)) < 1000) {
				level.spawnSpirit((int)x, (int)y);
				clock = System.currentTimeMillis();
			}
		}
	}
